...
uint8_t toByte(json_spirit::mValue const& _v)
{
    switch (_v.type())
    {
    case json_spirit::str_type:
        return (uint8_t)stoi(_v.get_str());
    case json_spirit::int_type:
        return (uint8_t)_v.get_uint64();
    case json_spirit::bool_type:
        return (uint8_t)_v.get_bool();
    case json_spirit::real_type:
        return (uint8_t)_v.get_real();
    default:
        cwarn << "Bad type for scalar: " << _v.type();
    }
    return 0;
...
void copyFile(fs::path const& _source, fs::path const& _destination)
{
	_source.string();
	_destination.string();
   // ifstream src(_source, ios::binary);
    //ofstream dst(_destination, ios::binary);
    //dst << src.rdbuf();
}
