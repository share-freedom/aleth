...
bool Ethash::quickVerifySeal(BlockHeader const& _blockHeader) const
{
    ...
    //ethash::verify_final_hash_against_difficulty(toEthash(h), toEthash(m), toEthash(n), toEthash(b));
	std::error_code code = ethash::verify_final_hash_against_difficulty(toEthash(h), toEthash(m), toEthash(n), toEthash(b));
	if (code.value() == 0)
		return true;
	else
		return false;
...
bool Ethash::verifySeal(BlockHeader const& _blockHeader) const
{
    ...
    //ethash::verify(context, toEthash(h), toEthash(m), toEthash(n), toEthash(b));
	std::error_code code = ethash::verify_against_boundary(context, toEthash(h), toEthash(m), toEthash(n), toEthash(b));
	if (code.value() == 0)
		return true;
	else
		return false;
