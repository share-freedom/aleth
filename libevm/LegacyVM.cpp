...
uint64_t LegacyVM::decodeJumpDest(const uint8_t* const _code, uint64_t& _pc)
{
...
uint64_t LegacyVM::decodeJumpvDest(const uint8_t* const _code, uint64_t& _pc, uint8_t _voff)
{
    ...
    //
    uint64_t pc = _pc;
    uint8_t n = _code[++pc];           // byte after opcode is number of jumps
...
void LegacyVM::interpretCases()
{
    ...
    DO_CASES
    {
        ...
        CASE(MSTORE8)
        {
            ...
            m_mem[(unsigned)m_SP[0]] = (uint8_t)(m_SP[1] & 0xff);
        }
        NEXT
        ...
        CASE(JUMPV)
        {
            ON_OP();
            updateIOGas();
            m_PC = decodeJumpvDest(m_code.data(), m_PC, uint8_t(m_SP[0]));
        }
        CONTINUE
        ...
        CASE(PUSHC)
        {
            ...
            auto const originalOp = static_cast<uint8_t>(Instruction::PUSH1) + m_code[m_PC + 3] + 1;
